...
import manh_constraint11 as const
...
def writeNoise(xN, yN, tN, LBL):
	noise = open('tf_label_unopt.txt', 'w')

	for i in range(len(xN)):
		line = str(xN[i]) + " " + str(yN[i]) + " " + str(tN[i]) + " " + str(int(LBL[i])) 
		noise.write(line)
		noise.write("\n")
...
if __name__ == '__main__':
	...
	# X = X[0:end]; Y = Y[0:end]; LBL = LBL[0:end]; THETA = THETA[0:end]
