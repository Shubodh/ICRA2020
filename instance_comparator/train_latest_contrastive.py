...
def get_input(c):
	...
	#C-C-1
		...
		bd = bc + random.uniform(-3.5, 3.5)
		lc = la + random.uniform(-3.0, 3.0)
		ld = lc + random.uniform(-3.5, 3.5)
		ret_1.append(la)
		ret_1.append(lb)
		ret_1.append(ba)
		ret_1.append(bb)
...
for epoch in range(num_epochs):
    ...
    if(epoch % 10 == 0):
    	...
    	torch.save(Amodel, './saved_l/latest_save_{0:05d}.pt'.format(epoch))
'''
...
MPObj = torch.load('./saved_l/latest_save_00040.pt').cuda()
print(x_test.shape)
idx = [0, 2, 1, 3, 4]
for z in range(x_test.shape[0] - 1):
	...
	for k in xrange(0, z - 1):
		...
		if(torch.dist(output_a.squeeze(), output_b.squeeze()) < 20.0):
		    if(mini > torch.dist(output_a.squeeze(), output_b.squeeze())):
                        node = k + 1
                        mini = torch.dist(output_a.squeeze(), output_b.squeeze())
	if(node != -1 and datapoint_1[4] != 1 and mini < 6):
		print('Comparison between node {} and node {}'.format(z + 1, node))
		print(mini)
	elif(node != -1 and datapoint_1[4] == 1):
		print('Comparison between node {} and node {}'.format(z + 1, node))
		print(mini)
		#f = open("output.txt", "a")
